...
#     number_of_ticks_display=20
    ...
    if bar_plot:  # fix y values, which have all become positive
        ...
        for i in range(len(df_yvals.columns.values)):
            ...
            fig.data[i].y = df_yvals[df_yvals.columns.values[i]].tolist()
...
def plotly_plot(
    ...
    for i in range(len(ycols)):
        ycol = ycols[i]
        b = pfs[i](x=td,y=df_in[ycol].values.tolist(),name=ycol,opacity=ops[i],yaxis='y' if ycol not in ya2c else 'y2')
        data.append(b)
